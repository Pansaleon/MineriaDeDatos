...
#mejores juegos por columna
columns_to_change = ['Times Listed', 'Number of Reviews', 'Plays',
                     'Playing', 'Backlogs', 'Wishlist']


for col in columns_to_change:
    tmp_col = list(map(lambda x: float(x.replace('K', '')) * 1000 if 'K' in x else float(x), data[col]))
    data[col] = tmp_col
data.head(2)
...
for count, col in enumerate(columns_to_change):
    ...
    indexes = data[col].sort_values(ascending=False)[:10].index
    tmp_df = data.loc[indexes][['Title', col]]
